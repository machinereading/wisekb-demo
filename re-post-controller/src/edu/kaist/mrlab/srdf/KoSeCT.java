...
public class KoSeCT {
	...
	public ArrayList<Chunker> getChunksWithSplitting(String input) {
...
//			String output1 = kare.callETRI(input);
			String output2 = ce.parse(output1);
			...
			for (int i = 0; i < segSentence.size(); i++) {
				...
				if (tempSTC.isContainsSBJ()) {
					...
					String output4 = ce.parse(output3);
...
//					chunker.printSTC2Console();
					...
					for (int j = 0; j < npChunks.size(); j++) {
						if (npChunks.get(j).getType().contains("SBJ") && !SBJflag) {
	...
	public ArrayList<Chunker> getChunksWithoutSplitting(String input) {
		ArrayList<Chunker> chunkers = new ArrayList<Chunker>();
		...
		try {
			...
			while ((input = filebr.readLine()) != null) {
				if (input.length() != 0) {
					...
					kosect.getChunksWithSplitting(input);
