...
public class DPWDChanger {
	...
	public void splitVNP(JSONObject targetDepen, long targetDepenHead, JSONArray targetDepenMod, long targetDepenID,
		...
		ArrayList<JSONObject> mal = new ArrayList<JSONObject>();
		Iterator<?> m = morpArr.iterator();

		while (m.hasNext()) {
			mal.add((JSONObject) m.next());
		}
		...
		for (int k = targetWordEnd; k >= targetWordBegin; k--) {
			JSONObject innerMorp = mal.get(k);
			String morpType = (String) innerMorp.get("type");
		...
		if (morpTypeArr.contains("VCP")) {
			...
			for (int k = targetWordEnd; k >= targetWordBegin; k--) {
				...
				if (VPFlag && (morpType.equals("VCP") || morpType.equals("VV") || morpType.equals("XSV"))) {
					VPFlag = false;
					VP = morpLemma + VP;
					if (isContinue) {
	...
	public static void main(String[] ar) {
		...
		try {
			...
			String output1 = ex.getResult("그가 레알 마드리드에서 뛸 때 팀의 \"공격 절차\"는 \"마켈렐레가 공을 빼앗은 뒤, 지네딘 지단에게 패스한다\"였다.");
